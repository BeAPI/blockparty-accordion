...
const FILES = {
	...
	mainPhp: 'blockparty-accordion.php',
...
function bumpMainPhp( newVersion ) {
	...
	contents = contents.replace(
		/(define\(\s*'BLOCKPARTY_ACCORDION_VERSION',\s*')[^']+(')/,
		`$1${ newVersion }$2`
	);

	const hasHeader = new RegExp( `\\* Version:\\s*${ newVersion.replace( /\./g, '\\.' ) }` ).test(
		contents
	);
	const hasConstant = contents.includes(
		`define( 'BLOCKPARTY_ACCORDION_VERSION', '${ newVersion }' )`
	);
